...
private:
	T m_data;
	int m_priorty; // least priorty 0 ----> more priorty
...
public:
	...
	void Insert_Before(LinkIterator<T>& it, T newData, int priorty)
	{
	...
	void Insert_After(LinkIterator<T>& it, T newData, int priorty)
	{
		assert(it.m_node != nullptr);
	...
	void Push_Front(T newData, int priorty)
	{
	...
	void Push_Back(T newData, int priorty)
	{
